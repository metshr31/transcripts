...
Environment:
  ...
  # Email secrets (set in GitHub repo Settings → Secrets → Actions)
...
def apply_filters(df_raw: pd.DataFrame, lookback_hours: int, strict_mode: int = 1) -> pd.DataFrame:
    ...
    df = df[~excl_mask]
    print(f"[DEBUG] Exclusion phrases: {before} -> {len(df)}")
    if df.empty: return df
...
def main():
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as xl: df.to
